...
```bash
sam-rust$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-rust$ sam local start-api
sam-rust$ curl http://localhost:3000/
```
...
```bash
sam-rust$ sam logs -n HelloWorldFunction --stack-name sam-rust --tail
```
